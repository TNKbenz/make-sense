...
class Editor extends React.Component<IProps, IState> {
  ...
  public componentDidMount(): void {
    ...
    if (objectData !== null) {
      ...
      const imgdata = LabelsSelector.getImagesData();
      console.log("getImagesData(): ", imgdata);
      setTimeout(() => {
        for (let i = 0; i < images.length; i++) {
          imgdata[i].labelRects = [];
          for (let j = 0; j < images[i].labels.length; j++) {
            let labelRect = YOLOUtils.parseYOLOAnnotationFromString(
              images[i].labels[j],
              labelNames,
              { width: images[i].width, height: images[i].height },
              imgdata[i].fileData.name
            );
            imgdata[i].labelRects.push(labelRect);
          }
        }
...
const mapStateToProps = (state: AppState) => ({
  ...
  objectData: state.labels.objectData,
});
